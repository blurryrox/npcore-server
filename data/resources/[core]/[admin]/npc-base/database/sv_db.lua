...
function NPC.DB.CreateNewPlayer(self, src, callback)
    local hexid = NPC.Util:GetHexId(src)
    callback = callback and callback or function() return end

    local data = {
        hexid = hexid,
        communityid = NPC.Util:HexIdToComId(hexid),
        steamid = NPC.Util:HexIdToSteamId(hexid),
        license = NPC.Util:GetLicense(src),
...
function NPC.DB.CreateNewCharacter(self, src, data2, hexid, ph, callback)
    local hexid = hexid
    callback = callback and callback or function() return end
...
function NPC.DB.PlayerExistsDB(self, src, callback)
    local hexId = NPC.Util:GetHexId(src)
    callback = callback and callback or function() return end
...
function NPC.DB.PhoneNumberExists(self, src, phone_number, callback)
...
function NPC.DB.FetchPlayerData(self, src, callback)
    local hexId = NPC.Util:GetHexId(src)
    callback = callback and callback or function() return end
...
function NPC.DB.FetchCharacterData(self, user, callback)
    callback = callback and callback or function() return end
...
function NPC.DB.DeleteCharacter(self, user, id, callback)
    callback = callback and callback or function() return end
...
function NPC.DB.UpdateCharacterStressLevel(self, user, characterId, newLevel, callback)
    callback = callback and callback or function() return end
    if not user then callback(false,true) return end
...
function NPC.DB.UpdateCharacterDirtyMoney(self, user, characterId, amount, callback)
callback = callback and callback or function() return end
    if not user then callback(false,true) return end
    if not characterId or type(characterId) ~= "number" then callback(false, true) return end
...
function NPC.DB.UpdateCharacterMoney(self, user, characterId, amount, callback)
    callback = callback and callback or function() return end
        if not user then callback(false,true) return end
        if not characterId or type(characterId) ~= "number" then callback(false, true) return end
...
function NPC.DB.UpdateCharacterBank(self, user, characterId, amount, callback)
    callback = callback and callback or function() return end
        if not user then callback(false,true) return end
        if not characterId or type(characterId) ~= "number" then callback(false, true) return end
...
function NPC.DB.UpdateControls(self, src, controlsTable, callback)
    callback = callback and callback or function() return end
        local user = exports["npc-base"]:getModule("Player"):GetUser(src)
        if not user then callback(false,true) return end
...
function NPC.DB.GetControls(self, src, callback)
    callback = callback and callback or function() return end
    local user = exports["npc-base"]:getModule("Player"):GetUser(src)
        Citizen.Wait(3000)
        if not user then callback(false, true) return end
...
function NPC.DB.UpdateSettings(self, src, settingsTable, callback)
    callback = callback and callback or function() return end
    local user = exports["npc-base"]:getModule("Player"):GetUser(src)
        if not user then callback(false,true) return end
...
function NPC.DB.GetSettings(self, src, callback)
    callback = callback and callback or function() return end
    local user = exports["npc-base"]:getModule("Player"):GetUser(src)
    if not user then callback(false, true) return end
...
-- 3.0
function NPC.DB.Something(self, src, callback)
    ...
    local q = [[
        ...
        if not results then callback(false, true) return end
            ...
            for i,v in ipairs(results) do
                ...
                -- There is more, but missing for now ((Sway))!
            end
		end)
	end
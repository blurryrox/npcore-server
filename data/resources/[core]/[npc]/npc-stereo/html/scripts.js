$(document).ready(function(){
  ...
  function closeGui() {
      if (Powered) {
        $.post('http://npc-stereo/close', JSON.stringify({ channel: stations[currentStation] }));
      } else {
        $.post('http://npc-stereo/cleanClose', JSON.stringify({ }));
      }
  }
  ...
  $("#power").click(function() {
    if (Powered === false) {
      Powered = true;
      $("#RadioChannel").val(stations[currentStation])
      $.post('http://npc-stereo/click', JSON.stringify({}));
      $.post('http://npc-stereo/poweredOn', JSON.stringify({ channel: stations[currentStation] }));
      $("#DisplayStation").html(stations[currentStation])
    } else {
      Powered = false;
      $.post('http://npc-stereo/click', JSON.stringify({}));
      $.post('http://npc-stereo/poweredOff', JSON.stringify({}));
  ...
  $("#volumeUp").click(function() {
    $.post('http://npc-stereo/click', JSON.stringify({}));
    $.post('http://npc-stereo/volumeUp', JSON.stringify({}));
  });
  
  $("#volumeDown").click(function() {
    $.post('http://npc-stereo/click', JSON.stringify({}));
    $.post('http://npc-stereo/volumeDown', JSON.stringify({}));
  });
  ...
  $("#radioChange").click(function() {
    ...
    if(Powered){
      ...
      $.post('http://npc-stereo/channelChange', JSON.stringify({ channel: stations[currentStation] }));
    }
    $.post('http://npc-stereo/click', JSON.stringify({}));
  });

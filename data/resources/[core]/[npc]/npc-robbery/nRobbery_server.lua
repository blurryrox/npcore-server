   ...
   -- print("door id: "..marker.attachedDoor)
    if marker.attachedDoor ~= nil and marker.attachedDoor ~= 0 and marker.attachedDoor ~= -22 then 
       -- print("i am about to unlock the door rn bud")
        ...
        TriggerEvent('npc-doors:ForceLockState', marker.attachedDoor, 0) end
    ...
    for k,v in pairs(markers) do
        if v.group == GroupType and v.ToolType == ELECTRICAL_LOCK then
            ...
            if v.attachedDoor ~= nil and v.attachedDoor ~= -22 and v.attachedDoor ~= 0 then
                TriggerClientEvent("npc-doors:ForceLockState",v.attachedDoor,0)
            end
        end
    end
end
...
function lockdown()
    ...
    TriggerEvent("npc-doors:ForceLockState",211,1)
    TriggerEvent("npc-doors:ForceLockState",212,1)
    TriggerEvent("npc-doors:ForceLockState",213,1)
    TriggerEvent("npc-doors:ForceLockState",214,1)
end

function endLockdown()
    TriggerClientEvent("jail:lockdown", -1, false)
    TriggerEvent("npc-doors:ForceLockState",211,0)
    TriggerEvent("npc-doors:ForceLockState",212,0)
    TriggerEvent("npc-doors:ForceLockState",213,0)
    TriggerEvent("npc-doors:ForceLockState",214,0)
end

function CheckLargeBanks()
    for k,v in pairs(markers) do
        if v.attachedDoor ~= nil and v.attachedDoor ~= -22 and v.attachedDoor ~= 0 then
            if not flags[k].isFinished and not exports["npc-doors"]:isDoorLocked(v.attachedDoor) then
                TriggerEvent("npc-doors:ForceLockState", v.attachedDoor,1)
            end
        end
...
-- RegisterServerEvent('npc-robbery:checkflag')
-- AddEventHandler('npc-robbery:checkflag', function(data)
--     -- Prison_Power
--     local check = false
...
RegisterServerEvent('npc-robbery:checkflag')
AddEventHandler('npc-robbery:checkflag', function()
    -- Prison_Power
   total = total + 1
  -- print(total)

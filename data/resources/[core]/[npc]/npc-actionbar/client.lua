...
RegisterNetEvent('npc-items:SetAmmo')
AddEventHandler('npc-items:SetAmmo', function(sentammoTable)
	...
	while true do
		...
		if IsPedShooting(ped) then
			...
			if throwableWeapons[weapon] then
				if exports["npc-inventory"]:hasEnoughOfItem(weapon,1,false) then
		...
		if IsPedPlantingBomb(ped) then
			if exports["npc-inventory"]:hasEnoughOfItem("741814745",1,false) then
...
function updateAmmoClient()
	...
	TriggerEvent("npc-items:SetAmmo",ammoTable)
end
...
function updateAmmoNow()
	...
	if type(ammoType) == 'number' then 
		...
		TriggerServerEvent("npc-weapons:updateAmmo",newammo,ammoType,ammoTable)
...
RegisterNetEvent('npc-item:CheckClientAmmo')
AddEventHandler('npc-item:CheckClientAmmo', function(weapons)
	local ped = PlayerPedId()

...
AddEventHandler("RefuelCarServerReturn",function()
    ...
    refillVehicle()
    local finished = exports["npc-taskbar"]:taskBar(timer,"Refueling")
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if IsVehicleEngineOn(veh) then          
            ...
            if not exports["npc-inventory"]:hasEnoughOfItem("watch",1,false) then
                compass_on = false
            end
        end
    end
...
AddEventHandler('carHud:compass', function(table)
    if exports["npc-inventory"]:hasEnoughOfItem("watch",1,false) then

...
AddEventHandler( 'cell:doors', function(num)
    ...
    TriggerServerEvent("npc-doors:alterlockstate",tonumber(num))
end)
...
Citizen.CreateThread(function()  -- Calc door around player into an array , dist of 100 every 5 seconds
    ...
    plyCoords = GetEntityCoords(playerped)
    while true do
        for i = 1, #NPC.DoorCoords do
            local distance = #(plyCoords - vector3(NPC.DoorCoords[i]["x"], NPC.DoorCoords[i]["y"], NPC.DoorCoords[i]["z"]))
            if distance < distanceCheckStage1 then
                closeDoorsStage1[i] = NPC.DoorCoords[i]
            else
    ...
    while true do
        ...
        if currentTargetDist < distvariable and targetDoor ~= nil then    
            ...
            if cZ(objCoords["z"],targetDoor["z"],limit) then
                ...
                if(NPC.offSet[targetDoor["doorType"]]) then
                    if (type(targetDoor["doorType"]) == "number") then
                        local pos = NPC.offSet[targetDoor["doorType"]]
                ...
                if  IsControlJustReleased(1,  Controlkey["generalUse"][1]) then
                    if OpenCheck(curClosestNum) then
                        ...
                        if shortcheck > 1.7 and isKeyDoor(curClosestNum) then
                            ...
                            TriggerServerEvent("npc-doors:alterlockstate",curClosestNum)
                        end
                    end
                end 
            end     
...
function OpenCheck(curClosestNum)
    ...
    for k,v in pairs(NPC.rankCheck) do
...
function isKeyDoor(num)
    ...
    if NPC.DoorCoords[num]["doorType"] == "prop_gate_prison_01" then
        return false
    end
    if NPC.DoorCoords[num]["doorType"] == 1286392437 then
        return false
    end
    if NPC.DoorCoords[num]["doorType"] == "v_ilev_fin_vaultdoor" then
        return false
    end
    if NPC.DoorCoords[num]["doorType"] == -1603817716 then
        return false
    end
    if NPC.DoorCoords[num]["doorType"] == 1501451068 then
        return false
    end
    if NPC.DoorCoords[num]["doorType"] == -550347177 then
        return false
    end

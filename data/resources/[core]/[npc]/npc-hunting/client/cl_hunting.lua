...
Citizen.CreateThread(function()
    local result = RPC.execute("npc-hunting:getSettings")
...
AddEventHandler("npc-inventory:itemUsed", function(item)
    if item == "huntingbait" then
        ...
        local finished = exports["npc-taskbar"]:taskBar(15000, "Placing Bait")
    ...
    if item == "huntingknife" then
        ...
        TaskStartScenarioInPlace(PlayerPedId(), "WORLD_HUMAN_GARDENER_PLANT", 0, true)
        local finished = exports["npc-taskbar"]:taskBar(30000, "Preparing Animal")
        if finished ~= 100 then
            ClearPedTasksImmediately(PlayerPedId())
        ...
        local success = RPC.execute("npc-hunting:getSkinnedItem", PedToNet(myAnimal), mySpawn, illegalSpawn)
        if not success then

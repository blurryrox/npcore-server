local stashes = {}
local instash = 0
RegisterNetEvent('npc-stash:setInitialState')
AddEventHandler('npc-stash:setInitialState', function(stashesp)
...
CreateThread(function()
    TriggerServerEvent("npc-stash:fetchInitialState")
end)
...
RegisterNetEvent('npc-stash:getnearstash')
AddEventHandler('npc-stash:getnearstash', function()
    local closestDoorDistance, closestDoorId = 9999.9, -1
    local currentPos = GetEntityCoords(PlayerPedId())
    for id, handle in pairs(stashes) do
        local currentDoorDistance = #(stashes[id].StashEntry - currentPos)
...
RegisterNUICallback('complete', function(data, cb)
    ...
    cb('ok');
    TriggerEvent('npc-stash:getnearstash')
end)

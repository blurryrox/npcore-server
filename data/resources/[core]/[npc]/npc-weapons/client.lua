...
-- Pickup hashes seem to not return so this doesn't work.
...
RegisterNetEvent('npc-weapons:destroyGroundWeapon')
AddEventHandler('npc-weapons:destroyGroundWeapon', function()
	DeleteEntity(weapon)
end)

RegisterNetEvent('npc-weapons:allowDrop')
AddEventHandler('npc-weapons:allowDrop', function()
	canDrop = true
end)

RegisterNetEvent('npc-weapons:dropweapon')
AddEventHandler('npc-weapons:dropweapon', function(AmountOfCurrentWeapon,weaponModel,rowID)
    ...
    canDrop = false
	TriggerServerEvent("npc-weapons:dropWeapon",{pos.x,pos.y,pos.z+0.75},name,rowID)
...
RegisterNetEvent('npc-weapons:doHandOver')
AddEventHandler('npc-weapons:doHandOver', function(weapon)
  --GiveWeaponToPed(PlayerPedId(), tonumber(weapon), 0, 0, false)
  TriggerEvent("attachWeapons")
end)

RegisterNetEvent('npc-weapons:dropWeaponSearch')
AddEventHandler('npc-weapons:dropWeaponSearch', function(weapon)
	--RemoveWeaponFromPed(PlayerPedId(),tonumber(weapon))
  TriggerEvent("attachWeapons")
end)


RegisterNetEvent('npc-weapons:giveweapon')
AddEventHandler('npc-weapons:giveweapon', function(toPlayer,AmountOfCurrentWeapon,weaponModel,rowID)
	...
	if not IsPedInAnyVehicle(PlayerPedId(),true) then
		...
		if (playing ~= false) then
			...
			TriggerServerEvent("npc-weapons:giveweapon", toPlayer, name,rowID)
			Wait(1300)
			TriggerServerEvent("weaponshop:removeSingleWeapon",rowID)
			TriggerEvent("attachWeapons")
		else
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if invehicle == nil then
        	--print("Nil invehicle -- initiating scripts.")
        	invehicle = false
        	TriggerEvent("attachWeapons")
        	TriggerEvent("AnimSet:Set")
        end
...
AddEventHandler('attachWeapons', function()
	...
	local currentWeapons = exports["npc-inventory"]:GetCurrentWeapons() 
	weapcount = 1
	local curw = GetSelectedPedWeapon(PlayerPedId())

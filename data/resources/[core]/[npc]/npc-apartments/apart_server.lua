...
AddEventHandler('ReturnHouseKeys', function()
    ...
    exports.ghmattimysql:execute('SELECT * FROM houses WHERE `cid` = @id', { ['@id'] = char.id }, function(result)
        if result[1] ~= nil then
            for i = 1, #result do
                local Apartment Keys = json.decode(result[i].data)
...
AddEventHandler('housing:attemptsale', function(args,price,hid,model)
    ...
    local user = exports["npc-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    local target = exports["npc-base"]:getModule("Player"):GetUser(args)
    ...
    if target:getCash() >= price then
        ...
        TriggerClientEvent('DoShortHudText', args, 'Insufficient money.', 2)
    end
end)
...
AddEventHandler('housing:requestSpawnTable', function(table, id)
    ...
    local char = user:getCurrentCharacter()
        exports.ghmattimysql:execute('SELECT * FROM houses WHERE `id` = @id', { ['@id'] = id }, function(result)
            if result[1] ~= nil then
                print('Do not try any weird stuff.')
...
local house_id = hid
    exports.ghmattimysql:execute('SELECT * FROM houses WHERE id = ?', {hid}, function(result)
        ...
        TriggerClientEvent('UpdateCurrentHouseSpawns', src, hid, json.encode(info))
            ...
            print('I have a personal dwelling.')
    end)
end)
...
AddEventHandler('house:givekey', function(id, model, target)
    ...
    local user = exports["npc-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    local zUser = exports["npc-base"]:getModule("Player"):GetUser(target)
        ...
        if result[1].cid == char.id then  
            ...
            for i= 1, #mykeys do
                if mykeys[i] == tIdentifier then
                    TriggerClientEvent('DoLongHudText', src, 'You already gave a key to this person.')
                    TriggerClientEvent('DoLongHudText', target, 'You already received a key for this house.')
                    return
                end
            end
...
AddEventHandler('house:enterhouse', function(house_id,house_model,bool)
    ...
    exports.ghmattimysql:execute('SELECT * FROM houses WHERE id = ?', {house_id}, function(result)
        ...
        if result[1].cid == char.id then
            ...
            TriggerClientEvent('DoLongHudText', src, "You do not have a key for that house.", 2)
...
--                     shared[random] = {}
--                     local player = exports['npc-base']:getModule("Player"):getCharacterFromCid(v.cid)
...
--     local src = source
--     local player = exports['npc-base']:getModule("Player"):getCharacterFromCid(target)

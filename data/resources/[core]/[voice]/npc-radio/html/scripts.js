$(document).ready(function () {
  ...
  function closeGui() {
    if (Powered) {
      ...
      $.post("http://npc-radio/close", JSON.stringify({ channel: RadioChannel }));
    } else {
      $.post("http://npc-radio/cleanClose", JSON.stringify({}));
    }
  }
  ...
  $("#power").click(function () {
    if (Powered === false) {
      ...
      $("#RadioChannel").attr("placeholder", "100.0-999.9");
      $.post("http://npc-radio/click", JSON.stringify({}));
      $.post(
        "http://npc-radio/poweredOn",
        JSON.stringify({ channel: RadioChannel })
      );
    } else {
      Powered = false;
      $.post("http://npc-radio/click", JSON.stringify({}));
      $.post("http://npc-radio/poweredOff", JSON.stringify({}));
  ...
  $("#volumeUp").click(function () {
    $.post("http://npc-radio/click", JSON.stringify({}));
    $.post("http://npc-radio/volumeUp", JSON.stringify({}));
  });

  $("#volumeDown").click(function () {
    $.post("http://npc-radio/click", JSON.stringify({}));
    $.post("http://npc-radio/volumeDown", JSON.stringify({}));
  });

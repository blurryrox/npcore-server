...
AddEventHandler('police:jailGranted', function(args)
	...
	if not pdunit:getVar("job") == "police" then
		...
		exports["npc-log"]:AddLog("Exploiter", pdunit, "User Attempted to jail player while not on pd", {target = playerName, reason = reason, cid = cid, time = tonumber(args[2]), src= src})
		DropPlayer(src, "")
...
AddEventHandler("police:deletecrimes", function(target)
	...
	if not pdunit:getVar("job" =="police") then
		...
		exports["npc-log"]:AddLog("Exploiter", pdunit, "User Attempted delete crimes while not on duty", {target = target , src = src})
		DropPlayer(src, "")
...
AddEventHandler('police:dnaAsk', function(target)
    local src = source
    local user = exports["npc-base"]:getModule("Player"):GetUser(src)
    local player = exports["npc-base"]:getModule("Player"):GetUser(target)
	...
	if jobType == "police" or jobType == "ems" or jobType == "doctor" or jobType == "therapist" or jobType == "doc" then
		exports.ghmattimysql:execute("SELECT rank from jobs_whitelist WHERE job = @job AND cid = @cid",{['job'] = jobType, ["cid"] = arg}, function(result)
			if(result[1]) then
				...
				exports["npc-log"]:AddLog("HC Added Job","Character: "..char.id.." Changed Char: "..arg.." Job whitelisting: "..jobType.." Type: Added", {source = src, char = char.id})
	...
	if jobType == "police" or jobType == "ems" or jobType == "doctor" or jobType == "therapist" or jobType == "doc" then
		exports.ghmattimysql:execute("SELECT rank from jobs_whitelist WHERE job = @job AND cid = @cid",{['job'] = jobType, ["cid"] = arg}, function(result)
			if(result[1]) then
				...
				exports["npc-log"]:AddLog("HC Removed Job","Character: "..char.id.." Changed Char: "..arg.." Job whitelisting: "..jobType.." Type: Removed", {source = src, char = char.id})
...
function jailUser(player,reason,cid,time,src)
	local jailer = exports["npc-base"]:getModule("Player"):GetUser(src)
	...
	if not jailer:getVar("job") == "police" and not jailer:getVar("job") == "doc" then
		...
		exports["npc-log"]:AddLog("Exploiter", jailer, "User attempted to jail while not on pd; Reason: " ..tostring(reason), {target = player, reason = reason, cid = cid, time = time})
		DropPlayer(src, "")
		return
	end
	...
	TriggerEvent("npc-police:playerJailed", character)
...
AddEventHandler("911", function(args)
	...
	exports["npc-log"]:AddLog("911 Chat", user, caller .. ": " .. message, {})
...
AddEventHandler("911r", function(args)
	...
	local caller = tostring(char.first_name) .. " " .. tostring(char.last_name)
	exports["npc-log"]:AddLog("911r Chat", user, caller .. ": " .. message)
	...
	exports["npc-log"]:AddLog("911r Chat", user, char.first_name .. " " .. char.last_name .. ": ".. message, {target = target})
end)

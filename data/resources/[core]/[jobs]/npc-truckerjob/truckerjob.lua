  ...
  local vehFront = VehicleInFront()
    if vehFront > 0 then
      ...
      if GetEntityModel(vehFront) == GetHashKey(trailer) then
        local finished = exports["npc-taskbar"]:taskBar(25000,"Storing Trailer Contents")
      ...
      TriggerEvent("DoLongHudText", "Cannot find vehicle..",2)
  end  
end
...
Citizen.CreateThread(function()
  while true do
    ...
    Wait(1)
    if exports["npc-base"]:getModule("LocalPlayer"):getVar("job") == "trucker" and existingVeh ~= nil then
...
Citizen.CreateThread(function()
  while true do
    Wait(9000)
    if job ~= 0 and exports["npc-base"]:getModule("LocalPlayer"):getVar("job") == "trucker"  then
      currentJobPos = returnJobPos(job)
      checkDestruction()
    end
  end
    ...
    if currentJobs[currentJobPos] ~= nil and currentJobs[currentJobPos].JobType == "Shops" then
      ...
      if dist < 20 then
        ...
        if(distance ~= -1 and distance < 5) then
            ...
            TriggerServerEvent("npc-shops:checkOwner",GetPlayerServerId(t),currentJobs[currentJobPos].shopId)
        else
...
function spawnTruck(i)
      ...
      TriggerEvent("DoLongHudText","Use your Phone to check the job!",1)
      local model = `mule2`
      RequestModel(model)
      ...
      local finished = exports["npc-taskbar"]:taskBar(3000,"Checking Vehicle")
      if finished == 100 then
      SetVehicleEngineHealth(spawned_car, 1000.0)
      SetVehicleBodyHealth(spawned_car, 1000.0)
      SetVehiclePetrolTankHealth(spawned_car, 1000.0)
        ...
        if #(vector3(v[1],v[2],v[3]) - GetEntityCoords(PlayerPedId())) < 5.5 then
          ...
          if(IsControlJustPressed(1, Controlkey["generalUse"][1])) then
            ...
            if job ~= 0 then
              ...
              TriggerEvent("DoLongHudText","You need a Job Set - Sign on as trucker then Open your phone to select an available delivery.",2)
            end
